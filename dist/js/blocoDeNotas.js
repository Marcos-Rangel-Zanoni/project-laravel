...
function atribuirValor() {
    ...
    const titleValue = addNoteTitle.value.trim(); // Remover espaços em branco do início e do fim

    if (titleValue.length !== 0) { // Verificar se o título não está vazio
        yourNoteTitle.value = titleValue;
        yourNoteText.value = addNoteText.value;

        const formData = new FormData();
        formData.append('title', yourNoteTitle.value);
        formData.append('content', yourNoteText.value);
